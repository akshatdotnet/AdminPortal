...
builder.Services.AddHealthChecks().AddDbContextCheck<CouponDbContext>("coupon_db-db");
...
using (var scope = app.Services.CreateScope())
{
    ...
    var connStr = config.GetConnectionString("CouponDb")!;

    var mainConnStr = new NpgsqlConnectionStringBuilder(connStr)
        { Database = "identity_db" }.ConnectionString;
    ...
    try
    {
        ...
        await using var check = new NpgsqlCommand(
            "SELECT 1 FROM pg_database WHERE datname = 'coupon_db'", conn);
        var exists = await check.ExecuteScalarAsync();
        if (exists is null)
        {
            await using var create = new NpgsqlCommand("CREATE DATABASE coupon_db", conn);
            await create.ExecuteNonQueryAsync();
            logger.LogInformation("coupon_db database created.");
        }
        else
        {
            logger.LogInformation("coupon_db database already exists.");
        }
    }
    catch (Exception ex)
    {
        logger.LogError(ex, "Could not create coupon_db. Is PostgreSQL running? docker compose up -d postgres redis");
        throw;
    }
}
...
using (var scope = app.Services.CreateScope())
{
    ...
    catch (Exception ex) { logger.LogError(ex, "Failed to init Coupon DB tables."); throw; }
}

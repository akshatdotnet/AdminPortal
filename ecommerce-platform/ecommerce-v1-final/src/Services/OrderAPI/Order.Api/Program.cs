...
builder.Services.AddHttpClient<IProductServiceClient, HttpProductServiceClient>(c =>
    c.BaseAddress = new Uri(builder.Configuration["ServiceUrls:ProductApi"] ?? "http://localhost:5002"));
builder.Services.AddHttpClient<ICouponServiceClient, HttpCouponServiceClient>(c =>
    c.BaseAddress = new Uri(builder.Configuration["ServiceUrls:CouponApi"] ?? "http://localhost:5005"));

    ...
    : IRequestHandler<ValidateCouponCommand, Result<CouponValidationResult>>
{
        ...
        ValidateCouponCommand cmd, CancellationToken ct)
    {
        ...
        if (coupon is null)
            return Result.Failure<CouponValidationResult>(Error.NotFound("Coupon", cmd.Code));
        ...
        return Result.Success(new CouponValidationResult(
            coupon.Code, coupon.Description, dr.Value, coupon.DiscountType.ToString()));
    }
}

public sealed record CreateCouponCommand(
    string Code, string Description, string DiscountType, decimal DiscountValue,
    DateTime ValidFrom, DateTime ValidTo, decimal? MinimumOrderAmount,
    decimal? MaximumDiscountAmount, int? MaxUsageCount)
    ...
    : IRequestHandler<CreateCouponCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateCouponCommand cmd, CancellationToken ct)
    {
        if (await repo.CodeExistsAsync(cmd.Code, ct))
            return Result.Failure<Guid>(Error.Conflict("Coupon",
                $"Code '{cmd.Code}' already exists."));

        var discountType = Enum.Parse<DiscountType>(cmd.DiscountType, ignoreCase: true);
        var coupon = Coupon.Create(cmd.Code, cmd.Description, discountType,

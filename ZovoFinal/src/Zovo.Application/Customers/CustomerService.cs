...
public class CustomerService : ICustomerService
{
    ...
    //{
    //    var query = _uow.Customers.Query().AsNoTracking().Include(c => c.Orders);

    //    if (!string.IsNullOrWhiteSpace(q.Search))
    //        query = query.Where(c => c.FirstName.Contains(q.Search) ||
    //            c.LastName.Contains(q.Search) || c.Email.Contains(q.Search));

    //    if (!string.IsNullOrWhiteSpace(q.Status) && Enum.TryParse<CustomerStatus>(q.Status, out var cs))
    ...
    //            c.Id, $"{c.FirstName} {c.LastName}", c.Email, c.Phone, c.Status.ToString(),
    //            c.Orders.Count,
    //            c.Orders.Where(o => o.PaymentStatus == PaymentStatus.Paid).Sum(o => o.TotalAmount),
    //            c.CreatedAt)),
    ...
    public async Task<PagedResult<CustomerListItemDto>> GetPagedAsync(CustomerQueryParams q)
    {
        IQueryable<Customer> query = _uow.Customers
            .Query()
            .AsNoTracking()
            .Include(c => c.Orders);
    ...
    public async Task<CustomerDetailDto?> GetDetailAsync(int id)
    {
        ...
        return new CustomerDetailDto(
            c.Id, c.FirstName, c.LastName, c.Email, c.Phone, c.AvatarUrl,
            c.Status.ToString(), c.Notes, c.Orders.Count,
            c.Orders.Where(o => o.PaymentStatus == PaymentStatus.Paid).Sum(o => o.TotalAmount),
            c.CreatedAt,
    ...
    public async Task<Result<int>> CreateAsync(CreateCustomerCommand cmd)
    {
        ...
        var c = new Customer {
            ...
            Email = cmd.Email, Phone = cmd.Phone, Notes = cmd.Notes
        };
    ...
    public async Task<Result> UpdateAsync(UpdateCustomerCommand cmd)
    {
        ...
        c.FirstName = cmd.FirstName; c.LastName = cmd.LastName;
        c.Phone = cmd.Phone; c.Notes = cmd.Notes; c.Status = cmd.Status;
    ...
    public async Task<Result> ToggleStatusAsync(int id)
    {
        var c = await _uow.Customers.GetByIdAsync(id);
        if (c is null) return Result.Fail("Customer not found.");
        c.Status = c.Status == CustomerStatus.Active ? CustomerStatus.Inactive : CustomerStatus.Active;
        await _uow.Customers.UpdateAsync(c);
        await _uow.SaveChangesAsync();

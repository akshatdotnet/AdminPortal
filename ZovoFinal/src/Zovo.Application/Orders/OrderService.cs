...
public class OrderService : IOrderService
{
    ...
    //{
    //    var query = _uow.Orders.Query().AsNoTracking().Include(o => o.Customer);

    //    if (!string.IsNullOrWhiteSpace(q.Search))
    //        query = query.Where(o => o.OrderNumber.Contains(q.Search) ||
    ...
    //    return PagedResult<OrderListItemDto>.Create(items.Select(ToListItem), total, q.Page, q.PageSize);
    //}

    public async Task<PagedResult<OrderListItemDto>> GetPagedAsync(OrderQueryParams q)
    {
        IQueryable<Order> query = _uow.Orders
            .Query()
            .AsNoTracking()
        ...
        if (!string.IsNullOrWhiteSpace(q.Status) &&
            Enum.TryParse<OrderStatus>(q.Status, out var os))
        {
            query = query.Where(o => o.Status == os);
        }
        ...
        if (!string.IsNullOrWhiteSpace(q.PaymentStatus) &&
            Enum.TryParse<PaymentStatus>(q.PaymentStatus, out var ps))
        {
            query = query.Where(o => o.PaymentStatus == ps);
        }
    ...
    public async Task<Result<int>> CreateAsync(CreateOrderCommand cmd)
    {
        ...
        try
        {
            ...
            foreach (var line in cmd.Lines)
            {
                ...
                if (product is null) return Result<int>.Fail($"Product {line.ProductId} not found.");
                ...
                items.Add(new OrderItem {
                    ...
                    ProductSKU = product.SKU, Quantity = line.Quantity, UnitPrice = product.Price
                });
            }

            var shipping  = subTotal >= settings.FreeShippingThreshold ? 0 : settings.DefaultShippingCost;
            var taxAmount = Math.Round(subTotal * settings.TaxRate / 100, 2);

            var order = new Order {
                OrderNumber = await _uow.Orders.GenerateOrderNumberAsync(),
                CustomerId = cmd.CustomerId, ShippingAddressId = cmd.ShippingAddressId,
                SubTotal = subTotal, ShippingCost = shipping, TaxAmount = taxAmount,
                TotalAmount = subTotal + shipping + taxAmount,
                CouponCode = cmd.CouponCode, Notes = cmd.Notes, Items = items
            };
    ...
    public async Task<Result> UpdateStatusAsync(int id, OrderStatus status)
    {
        ...
        order.Status = status;
        if (status == OrderStatus.Shipped   && !order.ShippedAt.HasValue)   order.ShippedAt   = DateTime.UtcNow;
        if (status == OrderStatus.Delivered && !order.DeliveredAt.HasValue) order.DeliveredAt = DateTime.UtcNow;
        await _uow.Orders.UpdateAsync(order);
        await _uow.SaveChangesAsync();
        return Result.Ok($"Order {order.OrderNumber} → {status}.");
    }
    ...
    public async Task<Result> CancelAsync(int id)
    {
        ...
        if (order is null) return Result.Fail("Order not found.");
        if (order.Status == OrderStatus.Delivered) return Result.Fail("Cannot cancel a delivered order.");
        order.Status = OrderStatus.Cancelled;
        foreach (var item in order.Items)
        {
            var p = await _uow.Products.GetByIdAsync(item.ProductId);
            if (p is not null) { p.Stock += item.Quantity; await _uow.Products.UpdateAsync(p); }
        }
        await _uow.Orders.UpdateAsync(order);
        await _uow.SaveChangesAsync();
    ...
    private static OrderListItemDto ToListItem(Order o) => new(
        ...
        o.Customer?.Email ?? "", o.TotalAmount,
        o.Status.ToString(), o.PaymentStatus.ToString(), o.CreatedAt);

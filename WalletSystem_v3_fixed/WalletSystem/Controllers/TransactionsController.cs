...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var vm = new DashboardVM
        {
            ...
            TotalBalance     = (decimal)await _db.Wallets.SumAsync(w => (double)w.Balance),
            TodayTransactions = await _db.Transactions.CountAsync(t => t.CreatedAt >= today),
            TodayVolume      = (decimal)await _db.Transactions.Where(t => t.CreatedAt >= today && t.Status == TransactionStatus.Completed).SumAsync(t => (double)t.Amount),
